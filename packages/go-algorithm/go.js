import { range, isEqual, flatten, groupBy } from "lodash";

import { Success, Failure } from "./maybe";

// Reduce with initial state as second argument
let reduce = (collection, accumulator, reducer) =>
  collection.reduce(reducer, accumulator);
...
let mapIndex = (haystack, indexToReplace, mapFn) =>
  haystack.map((oldValue, index) =>
    index === indexToReplace ? mapFn(oldValue) : oldValue
  );
...
let applyAttackWave = ({ board, color }) => {
  ...
  let captured_groups = defending_groups.filter((group) =>
...
export let applyMove = (board, move) => {
  ...
  let secondWave = applyAttackWave({
    board: firstWave,
    color: move.color === 1 ? 2 : 1,
  });
...
export let transition = (state, move) => {
  ...
  return (
    ...
    applyMove(board, move)
      ...
      .flatMap((nextBoard) => {
        let duplicateBoard = state.boards.findIndex((b) =>
          isEqual(b, nextBoard)
        );
        if (duplicateBoard !== -1) {
          return Failure(
...
let getFieldsCapturedBy = (groupedByColor, board, color) => {
  let capturedGroups = groupedByColor[0].filter((group) =>
    isSurroundedBy(
      group,
      board,
      (stone) => stone === color || stone === Colors.NEUTRAL
    )
  );
